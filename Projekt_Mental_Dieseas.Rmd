...
element_text(size = 4))+ ggtitle("Mental Disorders", title)+
xlab("Disorders")+ ylab("Occurrence") })
...
# prevelence_sum$`10-14 years # Ad. 3 ### Przedstawienie progronz występowania danych chorób w następnych x latach 
...
prediction = lapply(prevelence_by_cause, function(x){ 
  for (i in 1:10){
    ...
    title = toString(unique(x[[i]]['cause']))
    print(x[[i]])
  } 
})
```
